...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Auth context will handle redirection to /setup-profile
    } catch (err: any) {
    ...
    <div className="flex min-h-screen flex-col items-center justify-center px-4 pt-36 pb-12">
      <div className="w-full max-w-md rounded-[32px] border border-[var(--color-border-soft)] bg-[var(--color-surface)] p-6 shadow-2xl backdrop-blur-xl">
        <div className="text-center">
          <h1 className="font-display text-4xl text-[var(--color-ink)]">Create Account</h1>
          <p className="mt-3 text-sm text-[var(--color-muted)]">Join us and start your recovery journey</p>
        </div>
        ...
        <p className="mt-8 text-center text-sm text-[var(--color-muted)]">
          ...
          <Link href="/login" className="font-semibold text-[var(--color-ink)] hover:underline">
            Log in
          </Link>
        </p>
      </div>
